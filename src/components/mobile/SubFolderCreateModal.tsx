...
interface SubFolderCreateModalProps {
  isOpen: boolean;
  onClose: () => void;
}
...
const SubFolderCreateModal: React.FC<SubFolderCreateModalProps> = ({ isOpen, onClose }) => {
  const [name, setName] = useState('');
  const [selectedColor, setSelectedColor] = useState(SUBFOLDER_COLORS[0]);
  
  // const { addSubFolder } = useNotebookStore();
  ...
  const handleSave = () => {
    if (name.trim()) {
      ...
      //   color: selectedColor
      // });
      console.log('Create subfolder:', { name: name.trim(), color: selectedColor });
      successFeedback();
      handleClose();
    }
  };
  ...
  const handleClose = () => {
    setName('');
    setSelectedColor(SUBFOLDER_COLORS[0]);
    onClose();
  };
