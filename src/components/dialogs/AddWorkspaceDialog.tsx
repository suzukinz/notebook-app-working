...
interface AddWorkspaceDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onAdd: (workspace: { name: string; icon: string; color: string }) => void;
}
...
const AddWorkspaceDialog: React.FC<AddWorkspaceDialogProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!nameValidation.isValid) {
      setValidationError(nameValidation.error || 'ワークスペース名が無効です');
      return;
    }

    const sanitizedName = nameValidation.sanitized || workspaceName.trim();
          ...
          <div className="grid grid-cols-6 gap-2">
            {commonIcons.map(icon => (
              <button
                ...
                aria-label={`アイコンを${icon}に設定`}
                title={`アイコン: ${icon}`}
                aria-pressed={workspaceIcon === icon}
              >
                {icon}
              </button>
            ))}
          ...
          <div className="flex flex-wrap gap-2">
            {COLORS.map(color => (
              <button
                ...
                title={color}
                aria-label={`カラーを${color}に設定`}
                aria-pressed={workspaceColor === color}
              >
